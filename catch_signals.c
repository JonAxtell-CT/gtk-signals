...
/* From https://askra.de/software/gtk-signals/x2992.html */
/*** compile-command: "gcc catch_signals.c -Wall `pkg-config --cflags --libs gtk+-3.0` -o catch_signals" ***/
...
gboolean deliver_signal(GIOChannel *source, GIOCondition cond, gpointer d)
{
     ...
     */
    while ((status = g_io_channel_read_chars(source, buf.chars, sizeof(int), &bytes_read, &error)) ==
           G_IO_STATUS_NORMAL)
    {
...
int main(int argc, char *argv[])
{
    ...
    gtk_table_attach(GTK_TABLE(table), label, 0, 1, 1, 2, GTK_SHRINK | GTK_EXPAND, GTK_SHRINK | GTK_EXPAND, 0, 0);
    ...
    g_io_channel_set_flags(g_signal_in, g_io_channel_get_flags(g_signal_in) | G_IO_FLAG_NONBLOCK, &error);
